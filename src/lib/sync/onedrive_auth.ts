...
const PKCE_KEY = "odv_pkce";

let _accessToken: string | null = null;
let _accessTokenExpiresAt = 0;

export function getOneDriveClientId(): string | undefined {
  return import.meta.env.VITE_ONEDRIVE_CLIENT_ID as string | undefined;
}

function getOneDriveConfig(): OneDriveConfig {
  const clientId = getOneDriveClientId();
  if (!clientId) throw new Error("VITE_ONEDRIVE_CLIENT_ID is not set");
...
export async function ensureToken(): Promise<SyncToken> {
  ...
  if (!persisted) throw new Error("Not authenticated with OneDrive");
